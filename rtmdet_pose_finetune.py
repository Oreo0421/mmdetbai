...
model = dict(
    ...
    pose_det_cfg=dict(
        ...
        max_per_img=200),
    pose_topk=50,
    pose_use_gt_box=False,  # 微调阶段用det box，适应真实检测框
    pose_head=dict(
...
# 冻结 backbone/neck/bbox_head，只训练 pose 相关参数
# 用更小的学习率微调
optim_wrapper = dict(
    type='OptimWrapper',
    optimizer=dict(type='AdamW', lr=2e-5, weight_decay=0.0001),  # lr缩小10倍
    clip_grad=dict(max_norm=10, norm_type=2),
    paramwise_cfg=dict(
        custom_keys={
            'backbone': dict(lr_mult=0.0),     # 冻结backbone
            'neck': dict(lr_mult=0.0),          # 冻结neck
            'bbox_head': dict(lr_mult=0.0),     # 冻结检测头
        }))
...
log_level = 'INFO'
# 加载第一阶段的best checkpoint
load_from = '/home/tbai/mmdetection/mmdetection/work_dirs/rtmdet_pose_0211/best_coco_bbox_mAP_epoch_48.pth'
resume = False

...
data_root = '/home/tbai/Desktop/sensir_coco_v10/'
num_classes = 1
img_scale = (384, 384)
max_epochs = 50

# ========= V13: Multi-Head Fall Detection =========
# Three heads sharing a GRU encoder, trained jointly:
#   1. action_head: 10-class action classification (CE + class weights)
#   2. state_head:  3-class posture state (upright/sit/lie)
#   3. fall_head:   binary fall detection (focal loss)
#
# Motivation: Falls are sometimes missed (FN) because 10-class CE
# treats fall classes equally with non-fall. The dedicated fall_head
# with focal loss provides stronger gradient for hard fall examples.
# The state_head helps distinguish sit vs lie (post-fall).
#
# Label mapping (automatic, internal to ActionMultiHead):
#   action_class -> state: {0,1,3,5}->upright, {2}->sit, {4,6,7,8,9}->lie
#   action_class -> fall:  {0-5}->0, {6-9}->1
#
# Uses bone features (V10): 6 bones x 6 features = 36 dim
# Loads V6 checkpoint (bbox=0.866, kpt=0.802), freezes det+pose.
...
model = dict(
    ...
    pose_head=dict(
        ...
        log_stats=True,
        log_interval=20),
    # ========= V13: Multi-Head Action Classification =========
    action_head=dict(
        type='ActionMultiHead',
        num_keypoints=6,            # 6 bones
        kpt_dim=6,                  # dx, dy, angle, length, d_angle, d_length
        embed_dim=64,
        hidden_dim=64,
        num_gru_layers=1,
        num_classes=10,             # 10-class action head
        num_states=3,               # state head: upright/sit/lie
        loss_weight=1.0,            # action CE loss weight
        state_loss_weight=0.5,      # state CE loss weight
        fall_loss_weight=2.0,       # fall focal loss weight
        fall_focal_alpha=0.25,      # focal loss alpha
        fall_focal_gamma=2.0,       # focal loss gamma
        class_weight=[1.0, 1.0, 1.0, 1.0, 2.0, 2.0,
                      8.0, 8.0, 8.0, 8.0],  # upweight fall classes
        dropout=0.1,
        temporal_residual=True,
        skip_gru_t1=False,
        bone_mode=True,
    ),
    freeze_det_pose=True,   # Freeze backbone/neck/bbox_head/pose_head, only train action heads
...
# (bbox=0.866, kpt=0.802). ActionMultiHead is fully new (random init).

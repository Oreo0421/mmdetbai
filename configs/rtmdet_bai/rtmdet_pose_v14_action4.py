...
# ========= V14: 4-Class Action + Binary Fall Detection =========
# Two-head architecture: 4-class action (standing/walking/sitting/lying)
#   + binary fall detection (BCE with pos_weight)
...
#
# V14b tuning (vs V14a):
#   - max_epochs: 50 → 15 (V12 peaked at epoch 4, avoid overfitting)
#   - lr: 0.0002 → 0.0001 (slower, more stable convergence)
#   - pos_weight: 3.0 → 2.0 (reduce false positives, improve precision)
#   - fall_loss_weight: 2.0 → 1.5 (less aggressive fall head)
#   - dropout: 0.1 → 0.2 (more regularization)
#   - weight_decay: 0.0001 → 0.001 (stronger regularization)
#   - save_best: 'auto' → 'action/fall_f1' (save by fall F1, not bbox mAP)
...
model = dict(
    ...
    action_head=dict(
        ...
        fall_loss_weight=1.5,   # binary fall BCE weight (was 2.0)
        pos_weight=2.0,         # falling class weight for BCE (was 3.0)
        dropout=0.2,            # more regularization (was 0.1)
        bone_mode=False,
        skip_gru_t1=False,

...
num_classes = 1
img_scale = (384, 384)
max_epochs = 15       # ← 原50, best在epoch6

# ========= V13b: Multi-Head (抗过拟合版) =========
# 改动 vs V13:
#   1. max_epochs: 50 → 15
#   2. dropout: 0.1 → 0.3
#   3. lr: 0.0002 → 0.0001
#   4. weight_decay: 0.0001 → 0.001
#   5. save_best: 按 action/fall_f1
#   6. state_loss_weight: 0.5 → 0.3  (辅助头权重降低, 避免抢梯度)
#   7. fall_loss_weight: 2.0 → 1.0   (同上)
...
model = dict(
    ...
    action_head=dict(
        ...
        num_states=3,
        loss_weight=1.0,
        state_loss_weight=0.3,      # ← 原0.5, 降低辅助头权重
        fall_loss_weight=1.0,       # ← 原2.0, 降低避免抢梯度
        fall_focal_alpha=0.25,
        fall_focal_gamma=2.0,

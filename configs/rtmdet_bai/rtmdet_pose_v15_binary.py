...
num_classes = 1
img_scale = (384, 384)
max_epochs = 15  # V12 peaked early, no need for 50

# ========= V15: V12 Binary Architecture + Focal Loss + Direction Rules =========
# V12 binary architecture (num_classes=1, dedicated to fall detection)
# + V14c loss improvements: focal loss, hard neg weighting, kpt_weight
# + Direction-based rules in inference (infer_v14.py)
#
# Key changes from V12:
#   - focal_gamma=2.0: focus on hard examples
#   - hard_neg_classes=[4,5]: lying_down & getting_up weighted 3x (confusable with falls)
#   - kpt_weight: emphasize head(1.5) and feet(1.2), de-emphasize hands(0.5)
#   - pos_weight=2.0 (was 3.0): focal loss handles imbalance, reduce pos_weight
#   - dropout=0.2 (was 0.1): slightly more regularization
...
model = dict(
    ...
    pose_head=dict(
        ...
        log_stats=True,
        log_interval=20),
    # ========= Action Head (V15: binary + focal + hard neg + kpt_weight) =========
    action_head=dict(
        type='ActionTemporalHead',
        num_keypoints=7,
        kpt_dim=5,                # x, y, vis, dx, dy
        num_classes=1,            # Pure binary (V12 architecture)
        loss_weight=2.0,          # Same as V12
        pos_weight=2.0,           # Reduced from 3.0: focal loss handles imbalance
        focal_gamma=2.0,          # V15: focal loss for hard example mining
        hard_neg_classes=[4, 5],  # V15: lying_down(4), getting_up(5) = hard negatives
        hard_neg_weight=3.0,      # V15: weight confusable classes 3x
        kpt_weight=[1.5, 1.0, 0.5, 0.5, 1.5, 1.2, 1.2],  # V15: head, shoulder, hand_R, hand_L, hips, foot_R, foot_L
...
default_hooks = dict(timer=dict(type='IterTimerHook'), logger=dict(type='LoggerHook', interval=20),
                     ...
                     checkpoint=dict(type='CheckpointHook', interval=1, save_best='action/fall_f1', rule='greater', max_keep_ckpts=3),

...
data_root = '/home/tbai/Desktop/V14dataset/'
val_ann = 'annotations/instances_val_senir.json'  # SenIR-only val for fair comparison
num_classes = 1
img_scale = (384, 384)
max_epochs = 15

# ========= V14 Synth: Fine-tune action head on combined SenIR + Synthetic data =========
# Load from V14b best checkpoint (pre-trained action_head with fall_f1=0.72)
# Train on mixed data, validate on SenIR-only (same as original V14 val)
...
model = dict(
    ...
    pose_head=dict(
        ...
        log_stats=True,
        log_interval=20),
    # ========= Action Classification Head (V14: 4-class action + binary fall) =========
    action_head=dict(
        type='ActionV14Head',
        num_keypoints=7,
        ...
        fall_loss_weight=1.5,   # binary fall BCE weight
    ...
    freeze_det_pose=True,       # Freeze backbone/neck/bbox_head/pose_head
...
val_dataloader = dict(
    ...
    dataset=dict(type='CocoPoseDataset', data_root=data_root, metainfo=custom_metainfo,
                 ann_file=val_ann, data_prefix=dict(img='images/'), test_mode=True, pipeline=test_pipeline))
...
log_level = 'INFO'
# Load V14b best checkpoint (backbone/neck/det/pose frozen + pre-trained action_head)
load_from = 'work_dirs/rtmdet_pose_v14b_tuned/best_action_fall_f1_epoch_11.pth'
resume = False

...
data_root = '/home/tbai/Desktop/sensir_coco_v12/'
num_classes = 1
img_scale = (384, 384)
max_epochs = 12       # ← 原50, best在epoch4-6, 12足够

# ========= V12c: Binary Fall Detection (温和抗过拟合版) =========
# V12b太激进(fall_f1=0.52), 原版V12 best=0.749@epoch6
# V12c只做最小改动:
#   1. max_epochs: 50 → 12 (早停, 不让过拟合发生)
#   2. dropout: 0.1 → 0.2 (温和正则, 不是0.3)
#   3. weight_decay: 0.0001 → 0.0003 (3x, 不是10x)
#   4. save_best: 按 action/fall_f1
#   5. lr / loss_weight / pos_weight 全部保持原版不变!
...
model = dict(
    ...
    action_head=dict(
        ...
        kpt_dim=5,
        num_classes=1,
        loss_weight=2.0,        # ← 保持原版! V12b降到1.0太激进
        pos_weight=3.0,
        dropout=0.2,            # ← 原0.1, 温和增加(V12b用0.3太多)
        bone_mode=False,
        skip_gru_t1=False,
...
optim_wrapper = dict(type='OptimWrapper',
    optimizer=dict(type='AdamW',
        lr=0.0002,              # ← 保持原版! V12b降到0.0001太慢
        weight_decay=0.0003),   # ← 原0.0001, 温和3x(V12b用0.001太强)

...
num_classes = 1
img_scale = (384, 384)
max_epochs = 15       # ← 原50, best在epoch6, 15足够

# ========= V12b: Binary Fall Detection (抗过拟合版) =========
# 改动:
#   1. max_epochs: 50 → 15 (早停)
#   2. dropout: 0.1 → 0.3 (更强正则化)
#   3. lr: 0.0002 → 0.0001 (慢学习, 减缓过拟合)
#   4. weight_decay: 0.0001 → 0.001 (更强L2正则)
#   5. save_best: 'auto'(bbox_mAP) → 'action/fall_f1' (按跌倒F1保存best)
#   6. loss_weight: 2.0 → 1.0 (降低action loss权重, 减缓过拟合)
...
model = dict(
    ...
    action_head=dict(
        ...
        kpt_dim=5,
        num_classes=1,
        loss_weight=1.0,        # ← 原2.0, 降低防过拟合
        pos_weight=3.0,
        dropout=0.3,            # ← 原0.1, 大幅增加dropout
        bone_mode=False,
        skip_gru_t1=False,
...
optim_wrapper = dict(type='OptimWrapper',
    optimizer=dict(type='AdamW',
        lr=0.0001,              # ← 原0.0002, 降低学习率
        weight_decay=0.001),    # ← 原0.0001, 10x更强L2正则
...
default_hooks = dict(timer=dict(type='IterTimerHook'), logger=dict(type='LoggerHook', interval=20),
                     ...
                     checkpoint=dict(type='CheckpointHook', interval=1,
                                     save_best='action/fall_f1',   # ← 按fall_f1保存best, 不再按bbox_mAP
                                     rule='greater',
                                     max_keep_ckpts=3),

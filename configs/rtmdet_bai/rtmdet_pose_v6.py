...
model = dict(
    ...
    pose_head=dict(
        ...
        feat_channels=128,
        # SimDR params
        simdr_scale=2,            # sub-pixel: 48 feat → 96 bins
        sigma_1d=1.5,             # 1D Gaussian target width
        simdr_loss_weight=5.0,    # KL divergence loss weight
        vis_loss_weight=1.0,      # BCE visibility loss weight
        occluded_weight=0.3,      # v=1 occluded keypoint weight
        match_iou_thr=0.1,
        log_stats=True,
        log_interval=20),

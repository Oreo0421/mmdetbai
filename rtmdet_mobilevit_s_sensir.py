...
model = dict(
    type='RTMDetWithPose',  # ✅ 改用新的 detector
    backbone=dict(
        _delete_=True,
        type='TimmMobileViT',
    ...
    pose_head=dict(
        type='HeatmapHead',
        num_keypoints=17,  # COCO 格式
        in_channels=96,    # 与 neck.out_channels 一致
        feat_channels=128,
        loss_keypoint=dict(
            type='KeypointMSELoss',
            use_target_weight=True,
            loss_weight=2.0,  # 可调整权重
        ),
    ),
)
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations', with_bbox=True, with_keypoints=True),  # ✅ 添加
    dict(type='Resize', scale=img_scale, keep_ratio=False),
    dict(type='RandomFlip', prob=0.5),
    dict(type='GenerateKeypointHeatmap'),  # ✅ 需要实现这个 transform

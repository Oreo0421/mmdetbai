...
model = dict(
    ...
    pose_head=dict(
        type='HeatmapHeadV4', num_keypoints=7, in_channels=96, feat_channels=128,
        upsample_factor=2,    # deconv上采样: 48→96x96 heatmap，减少量化误差
        sigma=3.0,            # 配合96x96，更宽容的训练目标
        match_iou_thr=0.1,
        log_stats=True,
        log_interval=20,
        # V4 双任务 loss 参数
        loc_loss_weight=5.0,      # 定位 loss 权重
        vis_loss_weight=1.0,      # 可见性分类 BCE 权重
        fg_weight=10.0,           # 前景像素加权 (peak区域×10)
        occluded_weight=0.3),     # 遮挡点定位权重 (v=1)
...
custom_metainfo = dict(
    classes=('human',),
    num_keypoints=K,
    sigmas=[0.25] * 7,  # 放宽OKS容差，匹配低分辨率IR (40x114)
)
